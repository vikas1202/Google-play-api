...
const apkdownloadlover = require('google-play-scraper');
...
router.get('/apps/', function (req, res, next) {
  ...
  apkdownloadlover.suggest({ term: req.query.suggest })
...
router.get('/apps/', function (req, res, next) {
  ...
  apkdownloadlover.list(req.query)
    .then((apps) => apps.map(cleanUrls(req)))
...
router.get('/apps/:appId/permissions', function (req, res, next) {
  const opts = Object.assign({ appId: req.params.appId }, req.query);
  apkdownloadlover.permissions(opts)
    .then(toList)
    .then(res.json.bind(res))
    .catch(next);
});
...
router.get('/developers/:devId/', function (req, res, next) {
  ...
  apkdownloadlover.developer(opts)
    .then((apps) => apps.map(cleanUrls(req)))
    .then((apps) => ({
